...
export const SliderItem = ({itemActive,id,images,altimages,buttonTypeTitle,title,desc,buttonActionTitle,buttonActionIcon}: SliderItemPropos) => {
    return (  
         <li className={cn("absolute inset-0 overflow-hidden after:absolute after:w-full after:h-full after:left-0 after:bottom-0 after:bg-slider", itemActive === id ? "opacity-100 z-10" : "opacity-0")}>
           ...
           {/*Description */}
           <div className={cn(" space-y-2 absolute left-[10%] top-[43%] w-[850px] max-w-[80%] z-10 text-white opacity-0",itemActive === id && "animate-show-content animation-delay-5")}><p className="text-2xl font-normal">{desc}</p></div>
        {/*Link*/}
